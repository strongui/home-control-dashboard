...
export default function syncWeather(): Promise<IWeatherBase> {
  function getWeather() {
    return axios.get(
      `${window.location.protocol}//api.openweathermap.org/data/2.5/weather?lat=46.8921&lon=-71.2732&mode=json&units=metric&APPID=a40aead15bf642aab90b78be6ff65135`
    );
  }

  function getForecast() {
    return axios.get(
      `${window.location.protocol}//api.openweathermap.org/data/2.5/forecast?lat=46.8921&lon=-71.2732&mode=json&units=metric&APPID=a40aead15bf642aab90b78be6ff65135`
    );
  }
